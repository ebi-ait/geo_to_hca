...
def get_pubmed_metadata(project_pubmed_id: str,iteration: int) -> []:
    ...
    """
    xml_content = SraUtils.request_pubmed_metadata(project_pubmed_id)
...
def get_bioproject_metadata(bioproject_accession: str) -> []:
    ...
    """
    xml_content = SraUtils.request_bioproject_metadata(bioproject_accession)
    project_name,project_title,project_description,project_pubmed_id = get_attribs.get_attributes_bioproject(xml_content)
...
def get_experimental_metadata(accessions: [],accession_type: str) -> [[],str]:
    ...
    if len(accessions) < 100:
        xml = SraUtils.request_accession_info(accessions,accession_type=accession_type)
        size = 'small'
        return xml,size
    else:
        size = 'large'
        ...
        for p in range(0,len(parts_list)):
            xml = SraUtils.request_accession_info(parts_list[p],accession_type=accession_type)
            xmls.append(xml)
        return xmls,size
