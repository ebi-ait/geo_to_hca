...
def get_sequence_file_tab_xls(srp_metadata_update: pd.DataFrame, workbook: object, tab_name: str) -> pd.DataFrame:
    ...
    for index, row in srp_metadata_update.iterrows():
        tab.append({'sequence_file.file_core.file_name': row['fastq_name'],
            ...
            'sequencing_protocol.protocol_core.protocol_id': '',
                          'process.process_core.process_id':row['Run']})
    tab = pd.DataFrame(tab).sort_values(by='sequence_file.insdc_run_accessions')
    return tab
...
def get_cell_suspension_tab_xls(srp_metadata_update: pd.DataFrame, workbook: object, tab_name: str) -> None:
    ...
    """
    tab = []
    experiments_dedup = list(set(srp_metadata_update['Experiment'].tolist()))
    for experiment in experiments_dedup:
        ...
        tab.append({'cell_suspension.biomaterial_core.biomaterial_id':experiment,
                            ...
                            'cell_suspension.biomaterial_core.ncbi_taxon_id':
                                list(srp_metadata_update.loc[srp_metadata_update['Experiment'] == experiment]['TaxID'])[
                                    0],
                            'cell_suspension.genus_species.text': list(
                                srp_metadata_update.loc[srp_metadata_update['Experiment'] == experiment][
                                    'ScientificName'])[0],
                         'cell_suspension.biomaterial_core.biosamples_accession':biosample})
    tab = pd.DataFrame(tab).sort_values(by='cell_suspension.biomaterial_core.biomaterial_id')
...
def process_specimen_from_organism(biosample_attribute_list: [], srp_metadata_update: pd.DataFrame) -> pd.DataFrame:
    """
    Fills Specimen from organism metadata fields where the required fields are available in the
    input biosample attribute list.
    """
...
def get_specimen_from_organism_tab_xls(srp_metadata_update: pd.DataFrame, workbook: object, nthreads: int,
                                       tab_name: str) -> None:
    """
    ...
    if attribute_lists:
        try:
            with utils.poolcontext(processes=nthreads) as pool:
                results = pool.map(partial(process_specimen_from_organism, srp_metadata_update=srp_metadata_update),
                                   attribute_lists)
        except KeyboardInterrupt:
            log.info("Process has been interrupted.")
            pool.terminate()
    if results:
        tab = pd.DataFrame(filter(lambda x:x, results))
        if not tab.empty:
...
def get_library_protocol_tab_xls(srp_metadata_update: pd.DataFrame, workbook: object, tab_name: str) -> [{}, []]:
    ...
    for attribute_list in attribute_lists:
        ...
        if library_protocol not in library_protocol_set:
            ...
            library_protocol_dict[experiment_accession] = {"library_protocol_id": library_protocol_id,
                                                           "library_protocol_description": library_protocol}
        ...
        if library_protocol in library_protocol_set:
            ...
            if not library_protocol_id:
                ...
                library_protocol_dict[experiment_accession] = {"library_protocol_id": library_protocol_id,
                                                               "library_protocol_description":
                                                                   library_protocol}
    utils.write_to_wb(workbook, tab_name, pd.DataFrame(tab))
...
def get_sequencing_protocol_tab_xls(workbook: object, attribute_lists: [], tab_name: str) -> {}:
    ...
    for attribute_list in attribute_lists:
        ...
        if sequencing_protocol_description not in sequencing_protocol_set:
            ...
            tab.append({'sequencing_protocol.protocol_core.protocol_id': sequencing_protocol_id,
                              'sequencing_protocol.instrument_manufacturer_model.text': instrument,
                              'sequencing_protocol.paired_end': paired_end,
                              'sequencing_protocol.method.text': method})
...
def update_sequence_file_tab_xls(sequence_file_tab: pd.DataFrame, library_protocol_dict: {},
                                 sequencing_protocol_dict: {}, workbook: object, tab_name: str) -> None:
    """
    ...
    for index, row in sequence_file_tab.iterrows():
        if row["cell_suspension.biomaterial_core.biomaterial_id"] in library_protocol_dict.keys():
            library_protocol_id_list.append(
                library_protocol_dict[row["cell_suspension.biomaterial_core.biomaterial_id"]]["library_protocol_id"])
        else:
            library_protocol_id_list.append('')
...
def get_project_main_tab_xls(srp_metadata_update: pd.DataFrame, workbook: object, geo_accession: str,
                             tab_name: str) -> []:
    """
    ...
    """
    study = srp_metadata_update['SRAStudy'][0]
    project = srp_metadata_update['BioProject'][0]
    try:
        tab = []
        bioproject = list(set(srp_metadata_update['BioProject'].tolist()))
        if len(bioproject) > 1:
        ...
        tab.append({'project.project_core.project_title': project_title,
                    ...
                    'project.insdc_project_accessions': project})
        utils.write_to_wb(workbook, tab_name, pd.DataFrame(tab))
    except AttributeError as e:
        log.error(f'problem with project accession {project}: {str(e)}')
        raise e
...
def get_project_publication_tab_xls(workbook: object, tab_name: str, project_pubmed_id: str) -> None:
    ...
    tab.append({'project.publications.authors': name_list,
...
def get_project_contributors_tab_xls(workbook: object, tab_name: str, project_pubmed_id: str) -> None:
    ...
    for author in author_list:
        ...
        tab.append({'project.contributors.name': name, 'project.contributors.institution': affiliation})
    utils.write_to_wb(workbook, tab_name, pd.DataFrame(tab))
...
def get_project_funders_tab_xls(workbook: object, tab_name: str, project_pubmed_id: str) -> None:
    ...
    for grant in grant_list:
        tab.append({'project.funders.grant_id': grant[0], 'project.funders.organization': grant[1]})
    utils.write_to_wb(workbook, tab_name, pd.DataFrame(tab))
